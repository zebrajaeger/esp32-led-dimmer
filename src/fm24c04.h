...
class FM24C04 {
 public:
  ...
  uint8_t read8(uint8_t framAddr);
  void write16(uint8_t framAddr, uint16_t value);
  uint16_t read16(uint8_t framAddr);
  void write32(uint8_t framAddr, uint32_t value);
  uint32_t read32(uint8_t framAddr);
   ...
   */
  bool validate(uint8_t from, uint8_t to, uint8_t checksumAddr);
  uint32_t calcChecksum(uint8_t from, uint8_t to);

 protected:
  void print_wire_result(uint8_t r);
