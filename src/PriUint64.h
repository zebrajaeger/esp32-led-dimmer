// thx to https://github.com/yoursunny/PriUint64
...
class PriUint64 : public Printable {
 public:
  explicit PriUint64(uint64_t value, int ignored = 0) : m_value(value) {}
 ...
 */
inline Print& operator<<(Print& p, uint64_t x) { return p << PriUint64<DEC>(x); }
...
class _BASED1 : public _BASED, public Printable {
 public:
  ...
  size_t printTo(Print& p) const override { return p.print(val, base); }
};

template <typename V, int Base,
          typename BaseCls = typename std::conditional<std::is_same<V, uint64_t>::value, PriUint64<Base>, _BASED1>::type>
